...
export declare type NonKeyInput = string | Blob | ArrayBuffer;
export declare type RawInput = NonKeyInput | CryptoKey;
export declare type KeyInput = RawInput | CryptoKeyPair;
...
interface BaseHostInterface<ClusterType, ContentType> {
    clusters: {
        [flexid: string]: ClusterType;
    };
    contents: {
        [flexid: string]: ContentType;
    };
}
interface BaseConfigInterface<SType = {
    data: string;
    note: string;
}> {
    baseUrl: string;
...
export interface ConfigInterface extends BaseConfigInterface {
    hosts: {
        [url: string]: BaseHostInterface<ConfigClusterInterface, ConfigContentInterface>;
    };
}
export interface ConfigInputInterface extends Partial<BaseConfigInterface<{
    data: string;
    note: string;
} | null>> {
    hosts?: {
        [url: string]: Partial<BaseHostInterface<ConfigClusterInterface<null> | null, ConfigContentInterface<null> | null>> | null;
    };
}
