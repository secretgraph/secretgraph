...
# Stolen from strawberry-django-plus. Remove when integrated in strawberry-graphql-django
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "schema", nargs=1, type=str, help="The schema location"
        )
        parser.add_argument(
    ...
    def handle(self, schema, path, check, **kwargs):
        try:
            schema_symbol = import_module_symbol(
                schema[0], default_symbol_name="schema"
            )
        ...
        if not isinstance(schema_symbol, Schema):
            raise CommandError(
                "The `schema` must be an instance of strawberry.Schema"
            )
        ...
        if check:
            if not path:
                raise CommandError(
                    "--path must be specified when using --check"
                )
            try:
