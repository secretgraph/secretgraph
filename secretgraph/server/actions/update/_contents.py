...
def _transform_info_tags(objdata):
    ...
    for i in set(objdata.get("info") or []):
        ...
        if i in {"public"}:
            ...
            continue
        elif i in {"type=PublicKey", "type=PrivateKey"}:
            assert key_type is None
            key_type = i.split("=", 1)[1]
    ...
    return info_tags, key_hashes, key_type
...
def _transform_key_into_dataobj(key_obj, key=None, content=None):
    ...
    return (
        ...
        },
        {
            ...
            "info": ["type=PrivateKey"].extend(info),
            "contentHash": None
...
def _update_or_create_content_or_key(
    ...
    if objdata.get("info") is not None:
        final_info_tags, key_hashes_info, key_type = \
            _transform_info_tags(objdata)
        if not is_key and key_type:
            raise ValueError("%s is an invalid tag (autogenerated)", key_type)
    else:
        key_hashes_info = set()
        key_type = None
    ...
    if is_key and len(key_hashes_info) < 1:
        raise ValueError(
            ">=1 key_hash info tags required for keys (for action key)"
        )
                ...
                if is_key:
                    ...
                    for i in secrets:
                        ...
                        if len(splitted) == 2 and splitted[0] != "":
                            has_action_secret
                            break
                    if has_action_secret:
                        content.info.update_or_create(tag="public")
                    else:
                        content.info.filter(tag="public").delete()
                else:
                    pkey_refs = ContentReference.objects.filter(
                        group="public_key",
                        source__info__tag="public"
                    )
                    if content.references.filter(
                        group="key",
                        target__references__in=pkey_refs
                    ):
                        content.info.update_or_create(tag="public")
                    else:
                        content.info.filter(tag="public").delete()
